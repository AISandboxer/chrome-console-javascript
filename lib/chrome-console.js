...
class ChromeConsole {
  ...
  async setupConsoleListening() {
    try {
      // Use Puppeteer's built-in CDP session
      ...
      // Helper function to format exception details
      const formatException = (exceptionDetails) => {
        if (!exceptionDetails) return 'Unknown error';
        ...
        if (stackTrace && stackTrace.callFrames && stackTrace.callFrames.length > 0) {
          message += '\n' + stackTrace.callFrames.map(frame => {
            ...
            const line = frame.lineNumber + 1; // CDP uses 0-based line numbers
      ...
      client.on('Runtime.exceptionThrown', (params) => {
        ...
        const message = formatException(exceptionDetails);
        console.log(chalk.red(`[CHROME CONSOLE ERROR] ${message}`));
      });
      
      // Listen for log entries (catches some messages not captured by consoleAPICalled)
